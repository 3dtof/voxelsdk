...
class VOXEL_NO_EXPORT USBSystemPrivate
{
  ...
  template <typename T>
  bool _getDeviceProperty(HANDLE devHandle, DWORD prop, Ptr<T> &result, tFunction<void(T &t)> init = nullptr);

  bool _iterateSetupAPI(LPGUID guid, tFunction<void(HDEVINFO devClassInfo, DeviceInfo &devInfo, ULONG hubIndex)> process);

  bool _iterateOverAllDevices(LPGUID guid, tFunction<void(HANDLE hubDevice, ULONG portIndex, const String &driverKeyName, DevicePtr &device)> process);

  void _enumerateHub(int busIndex, const String &hubIndex, const String &hubName, tFunction<void(HANDLE hubDevice, ULONG portIndex, const String &driverKeyName, DevicePtr &device)> process);
